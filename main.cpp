...
{    
    ...
    ImageDrawText(&cardImage, TextFormat("$ %i", type.moneyRevenue [0].amount), 10, height-65, fontSize, BLACK);
    ImageDrawText(&cardImage, TextFormat("* %i", type.victoryPoints[0].amount), 10, height-40, fontSize, BLACK);
    }
...
void DrawUI(int income, int victoryPoints, Player& player)
{
    ...
    {
    auto text = TextFormat(" %d <victory> ", victoryPoints);
    Vector2 TextSize = MeasureTextEx(GetFontDefault(), text, FONT_SIZE, 2);
    DrawRectangle(xOffset, 0, TextSize.x, TextSize.y+2*yOffset, UI_BG_HI);
    DrawText( text, xOffset, yOffset, FONT_SIZE, BLACK);
    }
    ...
    {
        const auto text = " Select 2 cards to be discarded ";
    ...
    {
        const auto text = " Select card to be built ";
        const auto textSize = MeasureTextEx(GetFontDefault(), text, FONT_SIZE, 2);
        DrawRectangle((GetScreenWidth()-textSize.x)/2, 0, textSize.x, textSize.y+2*yOffset, UI_BG_HI);
        DrawText(text,(GetScreenWidth()-textSize.x)/2, yOffset, FONT_SIZE, BLACK);
    ...
    case PlayerState::SELECT_PAYMENT:
    {
        ...
        const auto text = TextFormat("Select %d cards to pay for %s", toBeBuilt->cost, toBeBuilt->name);
        const auto textSize = MeasureTextEx(GetFontDefault(), text, FONT_SIZE, 2);
        DrawRectangle((GetScreenWidth()-textSize.x)/2, 0, textSize.x, textSize.y+2*yOffset, UI_BG_HI);
        DrawText(text,(GetScreenWidth()-textSize.x)/2, yOffset, FONT_SIZE, BLACK);
...
void ShowHand(Vector2 pos, int spread, Player& player)
{
    ...
    if(zoomedCard != nullptr)
    {
        ...
        DrawTextureEx(cardImages.at(zoomedCard), centerCard, 0,  CARD_ZOOM_SCALE, WHITE);
    }
}
void DrawCardFaceDown(int n)
{
...
int main()
{
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        BeginDrawing();
            ...
            ShowHand({(float)GetScreenWidth()/2, GetScreenHeight()-CARD_SIZE.y/2-20}, -40, currentPlayer);
                        
            DrawUI(currentPlayer.get_income(), currentPlayer.get_victoryPoints(), currentPlayer);
